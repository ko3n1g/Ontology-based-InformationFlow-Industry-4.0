...
_stardog() {
        ...
        cmds="data help namespace query reasoning version icv vcs"
        cmds_no_help="data namespace query reasoning version icv vcs"
        ns_cmds="add import list remove"
        icv_cmds="convert validate export explain"
        inf_cmds="consistency explain"
        data_cmds="add remove size export obfuscate"
        query_cmds="execute search status explain obfuscate"
        vcs_cmds="commit diff list query revert tag"
        ...
        then
                COMPREPLY=(${COMPREPLY[@]:-} $(compgen -W "$vcs_cmds" -- "$word"))
        elif [[ $prev == *stardog* ]] ;
        then
...
_stardog_admin() {
		...
		# grab values for these from help directly
        cmds="cluster db help icv license metadata query role server user version virtual"
        cmds_no_help="cluster db icv license metadata query role server user version virtual"
        ...
        db_cmds="backup copy create drop list migrate optimize online offline repair restore status"
        query_cmds="kill list status"
        cluster_cmds="info generate zkstart zkstop"
	    virtual_cmds="add import list mappings options remove"
